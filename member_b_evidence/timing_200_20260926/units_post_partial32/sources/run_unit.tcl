# Isolated PReLU and Q31 partial-product candidate: no synthesis or implementation.
...
set benches {tb_post_partial32 tb_post_partial32_shared}
if {[info exists argv] && [llength $argv]} { set benches $argv }
foreach tb $benches {
    if {$tb ni {tb_post_partial32 tb_post_partial32_shared}} {
...
set caller_dir [pwd]
file mkdir $workroot
puts "POST_PARTIAL32_SIM_TOOL_ROOT=$viv"
puts "POST_PARTIAL32_SIM_WORK=$workroot"

proc post_partial32_stage_runtime {viv snapdir} {
...
set status [catch {
    foreach tb $benches {
        ...
        exec "$bindir/xelab.bat" --nolog "worklib.$tb" -O0 -s $tb \
            > "$work/xelab.log" 2>@1
        post_partial32_stage_runtime $viv "$work/xsim.dir/$tb"
        ...
        if {$tb eq "tb_post_partial32"} {
            set marker POST_PARTIAL32_SCALAR_ALL_CONFIGS_PASS
            set config_marker {POST_PARTIAL32_SCALAR_CONFIG_PASS id=}
            set config_count 6
            if {[string first "REQUANT_ROUND64_PROBE_PASS" $simlog] < 0} {
                error "Missing full-INT64 arithmetic probe PASS in $work/xsim.log"
            }
        } else {
            set marker POST_PARTIAL32_SHARED_ALL_CONFIGS_PASS
            set config_marker {POST_PARTIAL32_SHARED_CONFIG_PASS id=}
            set config_count 5
        }
        ...
        puts "POST_PARTIAL32_BENCH_PASS=$tb"
    }
} message options]
cd $caller_dir
if {$status} { return -options $options $message }
puts "POST_PARTIAL32_ALL_UNIT_TESTS_PASS; work=$workroot"
