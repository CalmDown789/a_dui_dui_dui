...
// scheduled with two extra scalar pipeline cycles. Two-slot throughput must
// be remeasured; each slot stays occupied until its output is consumed.
module vector_postprocess_shared #(
    ...
    always @(posedge clk)begin
        if(rst)begin
            ...
            for(i=0;i<10;i=i+1)meta_group[i]<=0;
        end else begin
            meta_valid<={meta_valid[8:0],issue_valid};
            meta_slot<={meta_slot[8:0],issue_slot};
            meta_group[0]<=issue_group;
            for(i=1;i<10;i=i+1)meta_group[i]<=meta_group[i-1];
            ...
            if(issue_valid)begin
                // Dispatch uses the old low group at this edge; the shift
                // exposes the next group for the following issue cycle.
                // An input_fire in this cycle necessarily writes the other
                // slot: input requires empty, issue requires occupied.
            ...
            if(lane_valid[0])begin
                for(i=0;i<LANES;i=i+1)
                    output_slot[meta_slot[9]][(meta_group[9]*LANES+i)*OUT_W+:OUT_W]
                        <=lane_data[i*OUT_W+:OUT_W];
                completed[meta_slot[9]]<=completed[meta_slot[9]]+1'b1;
                if(completed[meta_slot[9]]==GROUPS-1)
                    ready_slot[meta_slot[9]]<=1;
            end
...
`ifndef SYNTHESIS
    always @(posedge clk)if(!rst)begin
        if(input_fire&&issue_valid&&(write_slot==issue_slot))
            $fatal(1,"post shift input/issue slot collision");
        if(output_fire&&issue_valid&&(read_slot==issue_slot))
            $fatal(1,"post shift released a slot still being issued");
        for(integer v=1;v<LANES;v=v+1)
            if(lane_valid[v]!==lane_valid[0])$fatal(1,"postprocess lane valid skew");
        if(lane_valid[0]&&!meta_valid[9])$fatal(1,"postprocess metadata latency mismatch");
    end
`endif
endmodule
