...
> **最新状态（2026-09-23 19:11，本机）**：此前的默认优化仿真 FAIL 已确认受
> XSim `xelab` 优化选项影响。使用 `-O0` 后 6×5 全层探针与 96×54 四组 A 整数
> Golden 全部逐字节匹配。960×540 全帧正在复跑；真实 B+C 综合、实现和上板尚未完成，
> 因此**总验收仍待完成**。下文原失败数字属于默认优化历史运行，不能替代当前全帧结果。
> 详见 `docs/B_REAL_XSIM_OPTIMIZATION_FINDING.md`。
