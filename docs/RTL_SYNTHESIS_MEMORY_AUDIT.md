...
已经完成两处小范围 C 侧 RTL 补丁：

1. 将 MAC lane 中两个运行时宽向量切片换为相位 `case`，每个 case arm 的索引在 elaboration 时为常量。这是解决综合内存异常的根因修复。
2. 针对布线后发现的 `issue_slot` 选择扇出路径，在 PReLU DSP 输入前寄存已选操作数，并将元数据流水延长一拍。这是时序路径隔离，不改运算结果与处理吞吐率。

上游 B 冻结副本未改，仿真和综合/实现都使用 C 侧补丁。完整 B+C 综合用时 2 分 38 秒，Vivado 报告峰值 3,165.832 MB；RTL Optimization Phase 2 在 53 秒、3,133.086 MB 完成。此前同阶段达到 30,381.059 MB 后没有 netlist/report。当前完成 place/route，Vivado 峰值 4,239.918 MB，所有 67,851 条可布线网络均已布通且无 routing error。

200 MHz 时序仍未收敛：当前布线 WNS=-3.348 ns、TNS=-89,415.367 ns。隔离拍让原先的 issue-slot 选择路径不再是最差路径，但最差路径现在经过 PReLU Q15 舍入/饱和逻辑后进入 Q31 DSP，仍有 16 级 CARRY4。这个结果可用于确认综合规模和资源，不是 200 MHz 时序通过或板级验收结论。
...
|---|---:|---:|---:|---:|---:|---:|---:|
| Feature L1 | 5/1/16 | 1/2 | 50 | 62 | 4,524 | 4,818 | 0/4 |
| Shrink L2 | 1/16/8 | 2/8 | 16 | 22 | 2,238 | 2,084 | 0/0 |
| Mapping L3 | 3/8/8 | 1/8 | 72 | 78 | 5,120 | 6,384 | 8/0 |
| Expand L4 | 1/8/16 | 1/16 | 16 | 28 | 3,687 | 3,328 | 0/0 |
| Deconvolution L5 | 5/16/4 | 2/4 | 200 | 204 | 13,998 | 19,933 | 28/4 |
| **整机 B+C** |  |  | **354 卷积乘法 lane** | **394** | **32,339** | **37,061** | **230/8** |

额外 40 个 DSP 是各层后处理中的 PReLU/Q31 运算。器件总容量为 740 DSP、134,600 LUT、269,200 FF、365 个 RAMB36 tile；当前综合占用分别为 53.24%、24.03%、13.77%、64.11%（BRAM18 按半个 tile 折算）。资源没有超出 ACX750 200T 器件容量。
...
|---|---:|---:|
| Vivado 峰值内存 | RTL Optimization Phase 2 达到 30,381 MB，之后未完成 | 综合峰值 3,165.832 MB；place/route 峰值 4,239.918 MB |
| RTL Optimization Phase 2 | 8 分 42 秒后仍未完成，峰值 30,381 MB | 53 秒完成，峰值 3,133.086 MB |
| 系统 commit | 旧运行约 62.31/63.43 GB，余量约 1.12 GB | 整个复验过程采样峰值约 31.28/63.43 GB，观测余量至少约 32.15 GB |
| 综合时间 | 未完成；Phase 2 已耗时 8 分 42 秒 | `synth_design` 2 分 38 秒；完整实现 `place_design` 2 分 41 秒、`route_design` 3 分 24 秒 |
| LUT/FF/DSP/BRAM | 无完整 netlist，无法测量 | 综合 32,339 / 37,061 / 394 / 230 RAMB36 + 8 RAMB18；布线后 31,848 / 38,065 / 394 / 230 + 8 |
...
综合有 0 critical warning、0 error；`sums_q` 的 3D-array warning 保留。当前实现的 200 MHz timing constraint 未满足（综合 WNS=-3.157 ns、TNS=-12,021.518 ns；布线 WNS=-3.348 ns、TNS=-89,415.367 ns，63,498 个 failing endpoints）。place/route 已完成，0 routing error，但失败路径仍经过 `prelu_product_s1_reg`、16 级 CARRY4 的 Q15 rounding/saturation 组合逻辑并进入 `requant_product_s3` DSP。前一版只有 MAC selector 补丁时，route WNS=-4.224 ns、TNS=-127,668.391 ns；增加操作数隔离拍后 WNS 改善约 0.876 ns、TNS 改善约 38,253 ns，但距 200 MHz 收敛仍差 3.348 ns。因此当前可以继续做图像级仿真和资源/配置检查，不能据此宣布 200 MHz 上板时序已通过；需要下一步拆分 Q15 舍入/饱和路径或重新评估目标时钟后再实现。
