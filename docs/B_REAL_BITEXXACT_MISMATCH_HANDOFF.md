...
> **复核更新（2026-09-23 19:11）**：上文记录的是 XSim 2022.2 默认 `xelab`
> 优化下的失败。原 RTL `ae29515`、ROM 与 Golden 均不变，切换到 `xelab -O0`
> 后 6×5 全层探针和 96×54 四组正式 C 路径对拍均为 0 失配。全尺寸 960×540
> 重跑正在进行；综合、实现和上板仍未验收。详细证据见
> `docs/B_REAL_XSIM_OPTIMIZATION_FINDING.md`。以下默认优化失败矩阵保留为历史证据，
> 不再作为当前仿真配置下的验收结论。
