...
> 结论一句话：**在本项目冻结的工具链（Vivado / XSim **2022.2**）上，B 的
> `b_core_real`@`ae29515` 五层真实 RTL **无法复现整数 Golden**；失败与几何尺寸、
> 背压、参数 ROM 装载均无关，且 **B 自己未改动的测试台 + B 自己的 Golden 生成脚本
> 也同样失败**。第 3 个输出字节起即出错。**
>
> 这是 §三（用小尺寸四组用例做唯一合法 bit-exact 判据）的阻塞项，
> 因此 v3.2.2 中「B 侧功能已验证」的表述**不得继续沿用**。
