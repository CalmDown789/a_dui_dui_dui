...
2026-09-21 分工和器件路线确定前，仓库曾提交 PYNQ-Z2、三层网络和 HLS 试验。
这些提交保留在 Git 历史中供追溯；**当前 `main` 的代码和验收口径以上述
ACX750 五层 FSRCNN 内容为准**，不能把早期 HLS 指标当成当前板级结果。

- `member-a`：A 的模型、量化与 Golden 来源。
- `acx750-rtl`：B 的早期 ACX750 算术、窗口和位精确原语验证。
- `member-b-five-layer-stream`：B 的五层流式 RTL 开发入口，见该分支的
  `acx750_rtl/README.md`。
- `member-b-2025-2-bc-trial`：B 在 C 工程上的独立 Vivado 2025.2 实验叠层；
  源码、真实 ROM 和时序报告用于对照，不是 C 的正式板级分支。
- `c-side-latest`：C 的板级集成与验证记录。

各分支仍保留自己的交付文件；这里仅说明入口，不将队友文件从任何分支删除。
