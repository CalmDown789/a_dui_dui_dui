...
// Functionally identical to the verified dot9_pipelined module.
// USE_DSP is attached only to the nine product registers, so this experiment
// targets the multipliers without asking Vivado to move the adder tree to DSPs.
module dot9_pipelined_dsp #(
    ...
    (* use_dsp = "no" *) reg signed [S1_W-1:0] s1_0;
    (* use_dsp = "no" *) reg signed [S1_W-1:0] s1_1;
    (* use_dsp = "no" *) reg signed [S1_W-1:0] s1_2;
    (* use_dsp = "no" *) reg signed [S1_W-1:0] s1_3;
    reg signed [PROD_W-1:0] p8_d1;
    reg                       valid_s1;

    (* use_dsp = "no" *) reg signed [S2_W-1:0] s2_0;
    (* use_dsp = "no" *) reg signed [S2_W-1:0] s2_1;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            s1_0 <= $signed({p0[PROD_W-1], p0})
                  + $signed({p1[PROD_W-1], p1});
            s1_1 <= $signed({p2[PROD_W-1], p2})
                  + $signed({p3[PROD_W-1], p3});
            s1_2 <= $signed({p4[PROD_W-1], p4})
                  + $signed({p5[PROD_W-1], p5});
            s1_3 <= $signed({p6[PROD_W-1], p6})
                  + $signed({p7[PROD_W-1], p7});
            p8_d1 <= p8;
            valid_s1 <= valid_s0;
        end
    end
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            s2_0 <= $signed({s1_0[S1_W-1], s1_0})
                  + $signed({s1_1[S1_W-1], s1_1});
            s2_1 <= $signed({s1_2[S1_W-1], s1_2})
                  + $signed({s1_3[S1_W-1], s1_3});
            p8_d2 <= p8_d1;
            valid_s2 <= valid_s1;
        end
    end
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            s3 <= $signed({s2_0[S2_W-1], s2_0})
                + $signed({s2_1[S2_W-1], s2_1});
            p8_d3 <= p8_d2;
            valid_s3 <= valid_s2;
        end
    end
