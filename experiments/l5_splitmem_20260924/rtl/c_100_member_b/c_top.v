// 成员B工作：100 MHz fallback, 50 MHz input ×24/12; source copied from c-side-latest@6b87af3.
//=============================================================================
...
//
// 本模块只做三件事：
//   1) 时钟生成（MMCM；50MHz × 24 / 12 = 100MHz）
//   2) 复位汇聚（按键 + locked 门控）
...
module c_top #(
    ...
    parameter integer CLK_HZ     = 100000000,      // 100 MHz fallback
    //--- UART ----------------------------------------------------------------
    ...
    parameter integer AUTO_START_CYCLES = 10_000_000   // @100MHz = 100 ms
) (
    ...
    //-------------------------------------------------------------------------
    // 1. 时钟生成：50MHz → 200MHz（参数化，100MHz 退路只改一个常量）
    //-------------------------------------------------------------------------
    wire clk_200;
    wire mmcm_locked;
    ...
    generate
        if (USE_MMCM != 0) begin : g_mmcm
            ...
            MMCME2_BASE #(
                ...
                .CLKOUT0_DIVIDE_F   (12.0),  // 50MHz × 24 / 12 = 100MHz
