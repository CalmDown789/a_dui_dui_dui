...
//-----------------------------------------------------------------------------
// 分工（v3.2.2 §五.4 / 成员B确认_v1.1 B-ARCH-12）：
...
module c_core #(
    ...
    reg           out_pipe_valid_q;
    reg  [PIXEL_W-1:0] out_pipe_data_q;
    reg           out_pipe_stripe_last_q, out_pipe_frame_last_q;

    // One-beat elastic register isolates the B pixel-shuffle BRAM read from
    // the C ping-pong BRAM write. B advances only when this stage can accept.
    assign out_ready = run & (~out_pipe_valid_q | out_stream_ready);

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            out_pipe_valid_q <= 1'b0;
            out_pipe_data_q <= {PIXEL_W{1'b0}};
            out_pipe_stripe_last_q <= 1'b0;
            out_pipe_frame_last_q <= 1'b0;
        end else if (out_ready) begin
            out_pipe_valid_q <= b_out_valid;
            if (b_out_valid) begin
                out_pipe_data_q <= b_out_data;
    ...
    output_stream #(
        ...
        .out_valid          (out_pipe_valid_q),
        .out_data           (out_pipe_data_q),
        .out_stripe_last    (out_pipe_stripe_last_q),
        .out_frame_last     (out_pipe_frame_last_q),
