// 成员B工作：150 MHz fallback, 50 MHz input ×24/8; source copied from c-side-latest@6b87af3.
//=============================================================================
...
//
// 本模块只做三件事：
//   1) 时钟生成（MMCM，参数化，保留 150MHz 降频退路：CLKOUT0_DIVIDE_F 6.0 → 8.0）
//   2) 复位汇聚（按键 + locked 门控）
...
module c_top #(
    ...
    parameter integer CLK_HZ     = 150000000,      // 150 MHz fallback
    //--- UART ----------------------------------------------------------------
    ...
    parameter integer AUTO_START_CYCLES = 15_000_000   // @150MHz = 100 ms
) (
    ...
    //-------------------------------------------------------------------------
    // 1. 时钟生成：50MHz → 200MHz（参数化，150MHz 退路只改一个常量）
    //-------------------------------------------------------------------------
    wire clk_200;
    wire mmcm_locked;
    ...
    generate
        if (USE_MMCM != 0) begin : g_mmcm
            ...
            MMCME2_BASE #(
                ...
                .CLKOUT0_DIVIDE_F   (8.0),  // 6.0 → 200MHz；8.0 → 150MHz
