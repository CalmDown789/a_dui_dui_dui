##############################################################################
# synth_bc_real.tcl —— ★真实 B 五层 RTL + C 侧外壳★ 的综合(/可选实现)取证
#-----------------------------------------------------------------------------
...
#
# 用法：
#   vivado -mode batch -source scripts/synth_bc_real.tcl
#   vivado -mode batch -source scripts/synth_bc_real.tcl -tclargs impl     # 追加实现
#
# 前置条件（脚本自动完成）：
...
set do_impl 0
set variant "l5splitmem"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36" }
...
if {$variant eq "acc36"} { set phase_acc_file "$experiment_dir/rtl/b/phase_accumulator_36.sv" }
...
if {![file exists "$rtl_dir/input_image_pattern.mem"]} {
    error "缺少 $rtl_dir/input_image_pattern.mem —— 先跑: python scripts/gen_input_mem.py pattern-full"
}
file copy -force "$rtl_dir/input_image_pattern.mem" "$stage/rtl/input_image_pattern.mem"
...
puts "== staged [llength $roms] packed ROMs + input pattern to $stage =="
foreach f [glob -nocomplain -types f -directory "$experiment_dir/fixtures" rom_bank_*.mem] { file copy -force $f "$stage/[file tail $f]" }
cd $stage
...
set use_real "OK"
if {$n_real < 1 || $n_stub > 0 || $n_top < 1 || $n_layer < 5} { set use_real "FAILED" }
...
puts $rf "----------------------------------------------------------"
puts $rf " reports : report/bc_real_synth/"
puts $rf "=========================================================="
