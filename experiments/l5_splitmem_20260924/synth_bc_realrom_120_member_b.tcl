# 成员B工作：120 MHz fallback，使用真实 A 输入 ROM，与 200 MHz 实验分开取证。
# 成员B工作：冻结的 A 输入 ROM bank16 实现取证
##############################################################################
# synth_bc_realrom_120_member_b.tcl —— 120 MHz 候选，真实 B+C+A 输入 ROM
#-----------------------------------------------------------------------------
...
#
# 用法（从仓库根目录）：
#   vivado -mode batch -source experiments/l5_splitmem_20260924/synth_bc_realrom_120_member_b.tcl -tclargs impl
#   vivado -mode batch -source experiments/l5_splitmem_20260924/synth_bc_realrom_120_member_b.tcl -tclargs impl acc36
#
# 前置条件（脚本自动完成）：
...
set target_clk_mhz 120
set do_impl 0
set variant "l5splitmem_realrom_120_member_b"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_120_member_b" }
...
set c_files [list \
    ...
    "$rtl_dir/b_core_if.v"     \
    $c_core_file        \
    "$experiment_dir/rtl/c_120_member_b/c_top.v"         \
    "$experiment_dir/rtl/c_120_member_b/c_synth_top.v"   \
]
