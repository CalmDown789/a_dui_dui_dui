...
##############################################################################
# synth_bc_realrom_member_b.tcl —— 真实 B 五层 + C 外壳 + 冻结 A 输入 ROM
#-----------------------------------------------------------------------------
...
#
# 用法（从仓库根目录）：
#   vivado -mode batch -source experiments/l5_splitmem_20260924/synth_bc_realrom_member_b.tcl -tclargs impl
#   vivado -mode batch -source experiments/l5_splitmem_20260924/synth_bc_realrom_member_b.tcl -tclargs impl acc36
#
# 前置条件（脚本自动完成）：
...
set do_impl 0
set variant "l5splitmem_realrom_member_b"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_member_b" }
if {[info exists argv] && [lsearch -exact $argv "ascii"] >= 0} { append variant "_ascii" }
set synth_extra {}
if {[info exists argv] && [lsearch -exact $argv "nocascade"] >= 0} {
    append variant "_nocascade"
    set synth_extra [list -max_bram_cascade_height 1]
}
...
synth_design -top $top_module -part $target_part \
    ...
    -verilog_define C_USE_B_REAL {*}$synth_extra
