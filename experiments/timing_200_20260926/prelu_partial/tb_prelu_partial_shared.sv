...
module prelu_partial_shared_case #(
    ...
    initial begin
        ...
        $display("PRELU_PARTIAL_SHARED_CONFIG_PASS id=%0d channels=%0d lanes=%0d signed=%0d prelu=%0d inputs=%0d outputs=%0d in_stall=%0d out_stall=%0d simultaneous=%0d gaps=%0d cycles=%0d",
...
module tb_prelu_partial_shared;
    reg clk = 0;
    always #5 clk = ~clk;
    wire [4:0] done;
    // These three distinct configurations cover L1/L4, L2/L3, and L5.
    prelu_partial_shared_case #(.ID(0), .CHANNELS(16), .LANES(2), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c0 (.clk(clk), .done(done[0]));
    prelu_partial_shared_case #(.ID(1), .CHANNELS(8), .LANES(1), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c1 (.clk(clk), .done(done[1]));
    prelu_partial_shared_case #(.ID(2), .CHANNELS(4), .LANES(1), .OUT_W(8),
        .OUT_SIGNED(0), .APPLY_PRELU(0)) c2 (.clk(clk), .done(done[2]));
    // GROUPS=1 and GROUPS=2 boundaries exercise fast slot reuse.
    prelu_partial_shared_case #(.ID(3), .CHANNELS(1), .LANES(1), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c3 (.clk(clk), .done(done[3]));
    prelu_partial_shared_case #(.ID(4), .CHANNELS(4), .LANES(2), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c4 (.clk(clk), .done(done[4]));
    initial begin
        wait (done === 5'b11111);
        repeat (4) @(posedge clk);
        $display("PRELU_PARTIAL_SHARED_ALL_CONFIGS_PASS configs=5");
        $finish;
    end
    initial begin
        #500001;
        $fatal(1, "PRELU_PARTIAL_SHARED_WALL_CLOCK_TIMEOUT");
    end
endmodule
