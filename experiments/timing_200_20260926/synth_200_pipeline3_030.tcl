# Prepared only: L3+L5 buffers, explicit stripe banks, fixed-tap post slots.
# V3 candidate: early bias + L3/L5 partial buffers + explicit stripe banks
...
##############################################################################
# synth_200_pipeline3_030.tcl —— 200 MHz 候选，真实 B+C+A 输入 ROM
#-----------------------------------------------------------------------------
...
#
# 用法（从仓库根目录）：
#   vivado -mode batch -source experiments/timing_200_20260926/synth_200_pipeline3_030.tcl -tclargs impl acc36 ascii ramdecomp
...
set do_impl 0
set variant "l5splitmem_realrom_200_member_b_pipeline3_0300926"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_200_member_b_pipeline3_0300926" }
...
if {$setup_tag eq "setup000"} {set variant [string map {pipeline3_030 pipeline3_000} $variant]}
...
puts $config_file "candidate_version=V3"
...
set b_files [list \
    ...
    $phase_acc_file                \
    $mac_issue_file            \
    "$script_dir/prelu_pipeline/vector_postprocess_shared.sv"        \
    $stream_layer_file        \
    ...
    "$script_dir/prelu_pipeline/prelu_requantize.sv"                 \
]
...
puts $rf " 4  B RTL              : V3 extra PReLU stage + early bias + L3/L5 buffer + shifted post slots + Q31 register; source_files.txt"
...
puts "MARGIN200_PIPELINE_V3_COMPLETE variant=$variant setup=$setup_tag implementation=$impl_status"
