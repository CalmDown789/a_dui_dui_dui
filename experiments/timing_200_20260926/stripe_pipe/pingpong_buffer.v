// Member B isolated candidate: one additional stripe RAM read pipeline stage.
// Original bank ownership/release sequencing is preserved. Response tags,
// including the bank selector, are independent of the live read-bank state.
//=============================================================================
...
//   读侧：UART 回读，rd_req 请求 1 字节，2 级同步读后 rd_valid 出数据
//
// 关键设计点（都是「不丢数据」所必需）：
...
module pingpong_buffer #(
    ...
    output wire              rd_valid,      // 接受 rd_req 后 2 级同步读有效
    ...
    reg               rd_bank_tag_q;
    reg               rd_v_pipe_q, rd_first_pipe_q, rd_last_pipe_q;
    reg               rd_bank_pipe_q;
    // ★ 回读释放握手（**单一驱动**：见下方说明）
    ...
    // Select the bank belonging to this response, not the live next request.
    wire [DATA_W-1:0] rd_data_mux = rd_bank_pipe_q ? rd_data_b1 : rd_data_b0;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            rd_bank_tag_q  <= 1'b0;
            rd_v_pipe_q    <= 1'b0;
            rd_first_pipe_q <= 1'b0;
            rd_last_pipe_q <= 1'b0;
            rd_bank_pipe_q <= 1'b0;
            rd_done_q      <= 1'b0;
            rd_done_bank_q <= 1'b0;
        end else begin
            // E0 accepts rd_req && rd_busy: RAM stage 1 and request tags.
            // E1 produces RAM stage 2 and response tags; consumer samples E2.
            rd_v_q     <= (rd_req & rd_busy_q);
            rd_first_q <= (rd_req & rd_busy_q & (rd_ptr_q == {ADDR_W{1'b0}}));
            rd_last_q  <= (rd_req & rd_busy_q & (rd_ptr_q == (rd_len_q - 1'b1)));
            if (rd_req & rd_busy_q) rd_bank_tag_q <= rd_bank_q;
            rd_v_pipe_q     <= rd_v_q;
            rd_first_pipe_q <= rd_first_q;
            rd_last_pipe_q  <= rd_last_q;
            if (rd_v_q) rd_bank_pipe_q <= rd_bank_tag_q;
            rd_done_q  <= 1'b0;   // 单拍脉冲
            ...
            if (!rd_busy_q && !rd_done_q) begin
                ...
                if (rd_ptr_q == (rd_len_q - 1'b1)) begin
                    ...
                    // Keep release timing unchanged: the write-control block
                    // clears bank_full at E1, when the last data has entered
                    // the new stripe output register. It is safe to reuse RAM
                    // after that edge. Do not delay done alone: the idle
                    // branch could then reacquire the still-full old bank.
                    rd_done_q      <= 1'b1;
    ...
    // 5. 输出
    //=========================================================================
    assign rd_valid = rd_v_pipe_q;
    assign rd_data  = rd_data_mux;
    assign rd_start = rd_first_pipe_q;
    assign rd_last  = rd_last_pipe_q;
