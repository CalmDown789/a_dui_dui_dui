...
// Independent arithmetic + external ready/valid scoreboard for the shared
// postprocess candidate. Parameters stay static: the DUT stores accumulators,
// but reads PReLU/Q31 parameters later when each channel group is dispatched.
module post_partial32_shared_case #(
    ...
    function automatic [31:0] accumulator_sample(
        ...
        begin
            ...
            case ((vector_id + channel_id*5) % 32)
                ...
                // Low-half bit 15 is data, not the accumulator sign. These
                // include negative high halves with nonzero/maximum lows.
                20: accumulator_sample = 32'h8000ffff;
                21: accumulator_sample = 32'hffff0001;
                22: accumulator_sample = 32'hfffeffff;
                23: accumulator_sample = 32'h7fff8000;
                24: accumulator_sample = 32'h0000ffff;
                25: accumulator_sample = 32'h80008000;
                26: accumulator_sample = 32'h80000001;
                27: accumulator_sample = 32'hffff8001;
                default: accumulator_sample = hash_value;
            endcase
            // Small distinct channels keep most answers away from saturation,
            // so group/slot metadata mistakes cannot hide behind clipped data.
    ...
    initial begin
        ...
        drain();
        send_vectors(64, 1000, 2, 32'h1f2e3d4c ^ ID);
        drain();
        ...
        if (max_pending != 2 || input_stalls < 96 || output_stalls < 64 ||
            ...
            reset_compute != 1 || reset_held_output != 1)
            $fatal(1, "Missing shared coverage id=%0d", ID);
        $display("POST_PARTIAL32_SHARED_CONFIG_PASS id=%0d channels=%0d lanes=%0d signed=%0d prelu=%0d inputs=%0d outputs=%0d in_stall=%0d out_stall=%0d simultaneous=%0d gaps=%0d cycles=%0d",
                 ID, CHANNELS, LANES, OUT_SIGNED, APPLY_PRELU, total_inputs,
                 total_outputs, input_stalls, output_stalls, simultaneous_io, input_gaps, cycles);
        done = 1;
    end
endmodule

module tb_post_partial32_shared;
    reg clk = 0;
    always #5 clk = ~clk;
    wire [4:0] done;
    // These three distinct configurations cover L1/L4, L2/L3, and L5.
    post_partial32_shared_case #(.ID(0), .CHANNELS(16), .LANES(2), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c0 (.clk(clk), .done(done[0]));
    post_partial32_shared_case #(.ID(1), .CHANNELS(8), .LANES(1), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c1 (.clk(clk), .done(done[1]));
    post_partial32_shared_case #(.ID(2), .CHANNELS(4), .LANES(1), .OUT_W(8),
        .OUT_SIGNED(0), .APPLY_PRELU(0)) c2 (.clk(clk), .done(done[2]));
    // GROUPS=1 and GROUPS=2 boundaries exercise fast slot reuse.
    post_partial32_shared_case #(.ID(3), .CHANNELS(1), .LANES(1), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c3 (.clk(clk), .done(done[3]));
    post_partial32_shared_case #(.ID(4), .CHANNELS(4), .LANES(2), .OUT_W(16),
        .OUT_SIGNED(1), .APPLY_PRELU(1)) c4 (.clk(clk), .done(done[4]));
    initial begin
        wait (done === 5'b11111);
        repeat (4) @(posedge clk);
        $display("POST_PARTIAL32_SHARED_ALL_CONFIGS_PASS configs=5");
        $finish;
    end
    initial begin
        #500001;
        $fatal(1, "POST_PARTIAL32_SHARED_WALL_CLOCK_TIMEOUT");
    end
endmodule
