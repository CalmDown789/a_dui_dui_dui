# V6: explicit PReLU and Q31 partial products; isolated pipeline candidate.
# V6 candidate: early bias + L3/L5 partial buffers + explicit stripe banks
# + fixed-tap postprocess slots + Q31 product register with matching metadata.
# Candidate: bias enters the accumulator at phase 0; frozen bias ROM required.
# Member B 2026-09-26: 200 MHz; +0.300 ns setup pressure through route, restore for signoff.
# 成员B工作：200 MHz 独立优化，使用真实 A 输入 ROM。
# 成员B工作：冻结的 A 输入 ROM bank16 实现取证
##############################################################################
# synth_200_pipeline6_030.tcl —— 200 MHz 候选，真实 B+C+A 输入 ROM
#-----------------------------------------------------------------------------
...
#
# 用法（从仓库根目录）：
#   vivado -mode batch -source experiments/timing_200_20260926/synth_200_pipeline6_030.tcl -tclargs impl acc36 ascii ramdecomp
# Optional: nominal (0 extra setup pressure), srl (L5 storage alternative).
#
# 前置条件（脚本自动完成）：
...
set setup_tag "setup030"
set setup_extra_ns 0.300
if {[info exists argv] && [lsearch -exact $argv nominal] >= 0} {
    set setup_tag "setup000"
    set setup_extra_ns 0.000
}
set do_impl 0
set variant "l5splitmem_realrom_200_member_b_pipeline6_0300926"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_200_member_b_pipeline6_0300926" }
...
set use_srl [expr {[info exists argv] && [lsearch -exact $argv srl] >= 0}]
if {$use_srl} {append variant "_srl"}
if {$setup_tag eq "setup000"} {set variant [string map {pipeline6_030 pipeline6_000} $variant]}
if {[info exists argv]} { foreach a $argv { if {$a eq "impl"} { set do_impl 1 } } }

if {!$use_ramdecomp || [lsearch -exact $argv acc36]<0} {error "Pipeline candidate requires acc36 ramdecomp"}
...
if {$use_srl} {set fifo_file "$script_dir/fifo_srl/elastic_fifo.sv"}
set mac_issue_file "$script_dir/mac_buffer_l3l5/mac_issue_stage.sv"
...
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set phase_acc_file "$script_dir/bias_early/phase_accumulator_36.sv" }
...
if {$use_ramdecomp} { set stripe_file "$script_dir/stripe_banked/stripe_buffer.v" }
...
file mkdir $out_dir
set config_file [open "$out_dir/run_config.txt" w]
puts $config_file "script=[file normalize [info script]]"
puts $config_file "argv=$argv"
puts $config_file "candidate_version=V6"
puts $config_file "variant=$variant"
puts $config_file "setup_tag=$setup_tag"
puts $config_file "extra_setup_ns=$setup_extra_ns"
puts $config_file "target_MHz=$target_clk_mhz"
puts $config_file "vivado=[version -short]"
close $config_file
...
if {[catch {exec git -C $root_dir status --porcelain -- {*}[list rtl tb scripts constr docs rom experiments] } g]} {
...
set c_files [list \
    ...
    $input_stream_file  \
    $stripe_file \
    "$script_dir/stripe_pipe/pingpong_buffer.v"\
...
set b_files [list \
    ...
    $phase_acc_file                \
    $mac_issue_file            \
    "$script_dir/post_partial32/vector_postprocess_shared.sv"        \
    $stream_layer_file        \
    ...
    "$script_dir/post_partial32/prelu_requantize.sv"                 \
]
foreach f $b_files { if {![file exists $f]} { error "缺少 B 真实 RTL: $f" } }
read_verilog -sv -verbose $b_files

read_xdc "$xdc_dir/c_top.xdc"
set mf [open "$out_dir/source_files.txt" w]
foreach f [concat $c_files $b_files [list "$xdc_dir/c_top.xdc"]] {puts $mf $f}
close $mf
...
# Record actual stripe BRAM output-register inference, not just RTL intent.
set bram_file [open "$out_dir/stripe_bram_registers.txt" w]
set stripe_brams [get_cells -quiet -hier -filter {REF_NAME =~ RAMB* && NAME =~ */u_pp/u_bank*}]
puts $bram_file "stripe_bram_count=[llength $stripe_brams]"
foreach b $stripe_brams {
    set row [list [get_property NAME $b] [get_property REF_NAME $b]]
    foreach prop {DOA_REG DOB_REG READ_WIDTH_A READ_WIDTH_B WRITE_WIDTH_A WRITE_WIDTH_B} {
        set value "not_available"
        catch {set value [get_property $prop $b]}
        lappend row "$prop=$value"
    }
    puts $bram_file $row
}
close $bram_file

# Record actual DSP pipeline properties for the PReLU mapping check.
set dsp_probe [open "$out_dir/post_dsp_pipeline.txt" w]
foreach cell [get_cells -quiet -hier -filter {NAME =~ */post/* && REF_NAME == DSP48E1}] {
    set row [list [get_property NAME $cell]]
    foreach prop {AREG BREG MREG PREG ACASCREG BCASCREG USE_MULT USE_SIMD} {
        lappend row "$prop=[get_property $prop $cell]"
    }
    puts $dsp_probe $row
    set pcin_nets [get_nets -quiet -of_objects [get_pins -quiet -of_objects $cell -filter {REF_PIN_NAME =~ PCIN*}]]
    set pcin_drivers [get_pins -quiet -leaf -of_objects $pcin_nets -filter {DIRECTION == OUT}]
    puts $dsp_probe "  PCIN_DRIVERS=[lsort -unique $pcin_drivers]"
}
close $dsp_probe

# Confirm that the intended complete-product boundary survives synthesis.
set full_regs [get_cells -quiet -hier -filter {NAME =~ *requant_product_full_s6_reg*}]
set full_file [open "$out_dir/requant_full_registers.txt" w]
puts $full_file "complete_product_cells=[llength $full_regs]"
foreach cell $full_regs {
    puts $full_file [list [get_property NAME $cell] [get_property REF_NAME $cell] \
        "DONT_TOUCH=[get_property DONT_TOUCH $cell]"]
}
close $full_file
if {[llength $full_regs] == 0} {error "Complete Q31 product register boundary is missing"}
set srl_report [open "$out_dir/l5_fifo_storage.txt" w]
set l5_storage [get_cells -quiet -hier -filter {NAME =~ *l5/gk.frontend/fifo/* && (REF_NAME =~ SRL* || REF_NAME =~ RAM*)}]
puts $srl_report "use_srl=$use_srl storage_cells=[llength $l5_storage]"
foreach cell $l5_storage {puts $srl_report [list [get_property NAME $cell] [get_property REF_NAME $cell]]}
close $srl_report
...
if {$do_impl} {
    ...
    set ok 1
    update_timing
    set cp [get_pins -quiet {u_top/g_mmcm.u_mmcm/CLKOUT0}]
    set cc [get_clocks -quiet -of_objects $cp]
    if {[llength $cc] != 1} {error "Expected exactly one MMCM output clock"}
    set period [get_property PERIOD $cc]
    if {abs($period - 5.000000) > 0.001} {error "Not the 200 MHz clock"}
    report_clocks -file "$out_dir/clocks_original.rpt"
    write_xdc -force "$out_dir/constraints_original.xdc"
    set_clock_uncertainty -setup -from $cc -to $cc $setup_extra_ns
    update_timing
    report_clocks -file "$out_dir/clocks_${setup_tag}.rpt"
    write_checkpoint -force "$stage/synth_${setup_tag}.dcp"
    if {[catch {opt_design} e]}                 { set ok 0; puts "opt_design FAILED: $e" }
    # AMD Vivado 2025.2 UG904 Performance_NetDelay_high strategy. Same RTL,
    # part, clock and ROM as the baseline; only physical directives differ.
    if {$ok && [catch {place_design -directive ExtraNetDelay_high} e]} { set ok 0; puts "place_design FAILED: $e" }
    if {$ok && [catch {phys_opt_design -directive AggressiveExplore} e]} { set ok 0; puts "phys_opt_design FAILED: $e" }
    if {$ok} {
        write_checkpoint -force "$stage/placed_${setup_tag}.dcp"
        report_timing_summary -file "$out_dir/timing_placed_${setup_tag}.rpt" -max_paths 10
        # Keep setup pressure through route; both constraint conditions are reported.

    }
    if {$ok && [catch {route_design -directive NoTimingRelaxation} e]} { set ok 0; puts "route_design FAILED: $e" }
    if {$ok} {
        report_timing_summary -file "$out_dir/timing_summary_route_${setup_tag}.rpt" -max_paths 20
        ...
        set full_regs_routed [get_cells -quiet -hier -filter {NAME =~ *requant_product_full_s6_reg*}]
        report_timing -to [get_pins -of_objects $full_regs_routed -filter {REF_PIN_NAME == D}] \
            -max_paths 14 -nworst 1 -file "$out_dir/requant_full_input_timing_postroute.rpt"
        set prelu_abs_regs [get_cells -quiet -hier -filter {NAME =~ *prelu_magnitude_s2_reg* && REF_NAME =~ FD*}]
        if {[llength $prelu_abs_regs]} {
            report_timing -to [get_pins -of_objects $prelu_abs_regs -filter {REF_PIN_NAME == D}] \
                -max_paths 14 -nworst 1 -file "$out_dir/prelu_abs_input_timing_postroute.rpt"
        }
        write_checkpoint -force "$stage/postroute.dcp"
        set impl_status "COMPLETED"
    } else { set impl_status "FAILED" }
...
puts $rf " 4  B RTL              : V6 PReLU/Q31 partial products + early bias + L3/L5 buffer + shifted post slots; source_files.txt"
...
puts $rf "13  candidate variant  : $variant"
puts $rf "14  route setup tag    : $setup_tag (extra $setup_extra_ns ns; restored to 0 for signoff)"
puts $rf "----------------------------------------------------------"
...
puts "MARGIN200_PIPELINE_V6_COMPLETE variant=$variant setup=$setup_tag implementation=$impl_status"
