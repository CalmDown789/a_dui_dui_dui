...
// scheduled with one extra scalar pipeline cycle. Two-slot throughput must
...
module vector_postprocess_shared #(
    ...
    always @(posedge clk)begin
        if(rst)begin
            ...
            for(i=0;i<9;i=i+1)meta_group[i]<=0;
        end else begin
            meta_valid<={meta_valid[7:0],issue_valid};
            meta_slot<={meta_slot[7:0],issue_slot};
            meta_group[0]<=issue_group;
            for(i=1;i<9;i=i+1)meta_group[i]<=meta_group[i-1];
            ...
            if(lane_valid[0])begin
                for(i=0;i<LANES;i=i+1)
                    output_slot[meta_slot[8]][(meta_group[8]*LANES+i)*OUT_W+:OUT_W]
                        <=lane_data[i*OUT_W+:OUT_W];
                completed[meta_slot[8]]<=completed[meta_slot[8]]+1'b1;
                if(completed[meta_slot[8]]==GROUPS-1)
                    ready_slot[meta_slot[8]]<=1;
            end
...
`ifndef SYNTHESIS
    always @(posedge clk)if(!rst)begin
        ...
        if(lane_valid[0]&&!meta_valid[8])$fatal(1,"postprocess metadata latency mismatch");
    end
`endif
endmodule
