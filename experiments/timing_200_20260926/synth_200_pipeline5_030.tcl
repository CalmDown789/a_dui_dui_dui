...
# V5 candidate: early bias + L3/L5 partial buffers + explicit stripe banks
...
##############################################################################
# synth_200_pipeline5_030.tcl —— 200 MHz 候选，真实 B+C+A 输入 ROM
#-----------------------------------------------------------------------------
...
#
# 用法（从仓库根目录）：
#   vivado -mode batch -source experiments/timing_200_20260926/synth_200_pipeline5_030.tcl -tclargs impl acc36 ascii ramdecomp
...
set do_impl 0
set variant "l5splitmem_realrom_200_member_b_pipeline5_0300926"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_200_member_b_pipeline5_0300926" }
...
if {$setup_tag eq "setup000"} {set variant [string map {pipeline5_030 pipeline5_000} $variant]}
...
puts $config_file "candidate_version=V5"
...
set b_files [list \
    ...
    $phase_acc_file                \
    $mac_issue_file            \
    "$script_dir/prelu_partial/vector_postprocess_shared.sv"        \
    $stream_layer_file        \
    ...
    "$script_dir/prelu_partial/prelu_requantize.sv"                 \
]
...
puts $rf " 4  B RTL              : V5 extra PReLU stage + early bias + L3/L5 buffer + shifted post slots + Q31 register; source_files.txt"
...
puts "MARGIN200_PIPELINE_V5_COMPLETE variant=$variant setup=$setup_tag implementation=$impl_status"
