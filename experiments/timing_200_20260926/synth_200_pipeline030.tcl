# Combined candidate: early bias + L5 partial buffer + stripe read pipeline
# and a complete Q31 product register with matching metadata latency.
...
##############################################################################
# synth_200_pipeline030.tcl —— 200 MHz 候选，真实 B+C+A 输入 ROM
#-----------------------------------------------------------------------------
...
#
# 用法（从仓库根目录）：
#   vivado -mode batch -source experiments/timing_200_20260926/synth_200_pipeline030.tcl -tclargs impl acc36 ascii ramdecomp
...
set do_impl 0
set variant "l5splitmem_realrom_200_member_b_pipeline0300926"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_200_member_b_pipeline0300926" }
...
if {$setup_tag eq "setup000"} {set variant [string map {pipeline030 pipeline000} $variant]}
...
set mac_issue_file "$script_dir/mac_buffer/mac_issue_stage.sv"
...
if {$use_ramdecomp} { set stripe_file "$script_dir/stripe_pipe/stripe_buffer.v" }
...
set b_files [list \
    ...
    $phase_acc_file                \
    $mac_issue_file            \
    "$script_dir/requant_pipe/vector_postprocess_shared.sv"        \
    $stream_layer_file        \
...
puts "MARGIN200_PIPELINE030_COMPLETE"
