...
module requant_scalar_checker #(
    ...
    initial begin
        ...
        $display("REQUANT_SCALAR_CONFIG_PASS id=%0d checked=%0d accepted=%0d",ID,checked,accepted);
        done=1;
    end
endmodule
...
module tb_requant_scalar;
    wire [6:0] done;
    requant_scalar_checker #(.ID(0),.OUT_W(16),.OUT_SIGNED(1),.APPLY_PRELU(1)) c0(done[0]);
    requant_scalar_checker #(.ID(1),.OUT_W(16),.OUT_SIGNED(1),.APPLY_PRELU(0)) c1(done[1]);
    requant_scalar_checker #(.ID(2),.OUT_W(8),.OUT_SIGNED(0),.APPLY_PRELU(0)) c2(done[2]);
    requant_scalar_checker #(.ID(3),.OUT_W(31),.OUT_SIGNED(1),.APPLY_PRELU(1)) c3(done[3]);
    requant_scalar_checker #(.ID(4),.OUT_W(31),.OUT_SIGNED(0),.APPLY_PRELU(0)) c4(done[4]);
    requant_scalar_checker #(.ID(5),.OUT_W(1),.OUT_SIGNED(1),.APPLY_PRELU(1)) c5(done[5]);
    requant_round64_probe c6(done[6]);
    initial begin
        wait(&done);
        $display("REQUANT_SCALAR_ALL_CONFIGS_PASS");
        $finish;
    end
    initial begin
        #100000;
