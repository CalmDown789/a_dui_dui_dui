...
##############################################################################
# synth_200_setup030.tcl —— 200 MHz 候选，真实 B+C+A 输入 ROM
#-----------------------------------------------------------------------------
...
#
# 用法（从仓库根目录）：
#   vivado -mode batch -source experiments/timing_200_20260926/synth_200_setup030.tcl -tclargs impl acc36 ascii ramdecomp
#
# 前置条件（脚本自动完成）：
...
set do_impl 0
set variant "l5splitmem_realrom_200_member_b_bias0300926"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_200_member_b_bias0300926" }
...
if {$do_impl} {
    ...
    update_timing
    report_clocks -file "$out_dir/clocks_setup030.rpt"
    ...
    if {$ok} {
        ...
        report_timing_summary -file "$out_dir/timing_placed_setup030.rpt" -max_paths 10
    ...
    if {$ok} {
        report_timing_summary -file "$out_dir/timing_summary_route_setup030.rpt" -max_paths 20
...
puts "MARGIN200_BIAS030_COMPLETE"
