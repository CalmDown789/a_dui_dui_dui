...
// phase. Bias is applied once at phase 7; signed36 covers eight signed32 partials plus signed32 bias.
module phase_accumulator_reference36 #(
    ...
    function automatic [31:0] sat_i32;
        ...
        begin
            if(value>36'sd2147483647)sat_i32=32'h7fffffff;
            else if(value< -36'sd2147483648)sat_i32=32'h80000000;
            ...
            if(out_stage_ready)begin
                ...
                if(sum_valid)
                    for(c=0;c<COUT;c=c+1)
                        out_data[c*32+:32]<=sat_i32(
                            sum_stage[c]+$signed(bias_flat[c*32+:32]));
            end
            if(sum_stage_ready)
...
`endif
                ...
                for(c=0;c<COUT;c=c+1)begin
                    next_sum=(phase==0)?36'sd0:accum[c];
                    if(c/OUT_PAR==group)
