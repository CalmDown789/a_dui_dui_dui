# Prepared only: validation for the second isolated pipeline candidate.
# Combined candidate regression. Real-B tests and C-shell tests are distinct.
# V3: early bias, L3/L5 partial buffers, explicit stripe banks, fixed-tap
# postprocess slots and the complete Q31 product pipeline.
# 成员B工作：C实验overlay精确源文件仿真，保留原脚本
##############################################################################
...
set sim_root   "$root_dir/_sim_l5_member_b_acc36_pipeline200_v3"
set pipeline_tag "pipeline200_v3"
set pipeline_fifo_file "$member_b_overlay_dir/rtl/b/elastic_fifo.sv"
if {[info exists argv] && [lsearch -exact $argv srl] >= 0} {
    append sim_root "_srl"
    append pipeline_tag "_srl"
    set pipeline_fifo_file "$script_dir/fifo_srl/elastic_fifo.sv"
}
...
proc stage_b_roms {work tb romdir} {
    ...
    foreach s $mems { file copy -force $s "$work/[file tail $s]" }
    set snap "$work/xsim.dir/$tb"
    if {[file isdirectory $snap]} {
        foreach s $mems { file copy -force $s "$snap/[file tail $s]" }
    }
...
proc stage_ref_data {work tb refdir pattern} {
    ...
    foreach s $files { file copy -force $s "$work/[file tail $s]" }
    set snap "$work/xsim.dir/$tb"
    if {[file isdirectory $snap]} {
        foreach s $files { file copy -force $s "$snap/[file tail $s]" }
    }
...
proc patch_xsim_dlls {work} {
    ...
    if {$viv eq "" || ![file isdirectory $viv]} { error "Invalid XILINX_VIVADO environment" }
    ...
    set named {xv_simulator_kernel.dll librdi_simulator_kernel.dll librdizlib.dll tcl85t.dll tcl86t.dll}
    # families: boost + the MSVC runtime (globbed so a minor version bump survives)
    set pats [list "$libdir/*boost*.dll" \
                   "$tpdir/MSVCP140*.dll" \
...
if {![file exists "$bin_dir/xvlog.bat"]} {
    error "No xvlog.bat under the selected Vivado environment: $bin_dir"
}
...
set rtl_files [list \
    ...
    "$script_dir/stripe_banked/stripe_buffer.v" \
...
set b_real_files [list \
    ...
    $pipeline_fifo_file                 \
    "$script_dir/mac_buffer_l3l5/mac_issue_stage.sv"              \
    "$member_b_overlay_dir/rtl/b/phase_mac_pipeline.sv"                 \
    "$script_dir/bias_early/phase_accumulator_36.sv"                  \
...
foreach tb $all_tbs {
    ...
    set bkind  "REAL V3 (early bias, L3/L5 buffer, banked stripe, shifted post slots, Q31 register)"
    ...
    puts "  defines       : $cdefs"
    set sf [open "$work/source_files.txt" w]
    foreach f [concat $bfiles $rtl_files [list "$tb_dir/$tb.v"]] {puts $sf $f}
    close $sf
    set df [open "$work/compile_defines.txt" w]
    puts $df $cdefs
    close $df
    ...
    if {$ok && [stage_b_roms $work $tb $rom_dir] != 19} {
        set ok 0; puts "missing staged parameter ROMs: $tb"
    }
    if {$ok} {
    ...
    if {$ok} {
        ...
        for {set attempt 1} {$attempt <= 4} {incr attempt} {
            if {[catch {exec "$bin_dir/xsim.bat" $tb -runall -log "$work/xsim_engine.log" > "$work/xsim.log" 2>@1} err]} {
    ...
    if {$ok && [file exists "$work/xsim.log"]} {
        ...
        if {$nfail > 0} { set pass 0; puts "  -> $nfail 条 FAIL" }
        if {[regexp -nocase {fatal:|error:|RESULT: FAIL} $c]} {set pass 0}
        if {[file exists "$work/xsim_engine.log"]} {
            set fh [open "$work/xsim_engine.log" r]
            set engine_text [read $fh]
            close $fh
            if {[regexp -nocase {fatal:|error:|RESULT: FAIL} $engine_text]} {set pass 0}
        }
    }
...
set rf [open "$root_dir/report/sim_result_l5_member_b_acc36_${pipeline_tag}.txt" w]
puts $rf "=============================================="
...
puts $rf " 7  B formal path      : $pipeline_tag; see each TB source_files.txt"
...
puts $rf "11  candidate        : V3 mac_buffer_l3l5 + post_shift + stripe_banked"
puts $rf "----------------------------------------------"
foreach tb $all_tbs {
...
puts $rf "----------------------------------------------"
puts $rf " raw logs : $sim_root/<tb>/xsim.log  (gitignored, regenerable)"
puts $rf "=============================================="
...
if {[llength $all_tbs] == 1} {
    ...
    file copy -force "$root_dir/report/sim_result_l5_member_b_acc36_${pipeline_tag}.txt" "$root_dir/report/sim_result_l5_member_b_acc36_${pipeline_tag}_${one_tb}.txt"
}

puts ""
puts " -> $root_dir/report/sim_result_l5_member_b_acc36_${pipeline_tag}.txt"
if {[llength $fail_list] != 0} {error "Pipeline regression failed: $fail_list"}
puts "MARGIN200_PIPELINE_V3_REGRESSION_COMPLETE tag=$pipeline_tag"
