...
set do_impl 0
set variant "l5splitmem_realrom_150_member_b_wlfanout"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_150_member_b_wlfanout" }
...
if {$do_impl} {
    ...
    # AMD Vivado 2025.2 UG904 Performance_WLBlockPlacementFanoutOpt.
    # Isolated implementation-strategy comparison against the same RTL.
    if {$ok && [catch {place_design -directive WLDrivenBlockPlacement} e]} { set ok 0; puts "place_design FAILED: $e" }
    if {$ok && [catch {phys_opt_design -directive AggressiveFanoutOpt} e]} { set ok 0; puts "phys_opt_design FAILED: $e" }
    if {$ok && [catch {route_design -directive Explore} e]} { set ok 0; puts "route_design FAILED: $e" }
