...
// RAM slices, each with local read and write pointers to limit LUTRAM
// address fanout. All slices advance together on the same push/pop events.
module elastic_fifo #(
    ...
    generate
        if (DATA_W > 1024) begin : g_wide_read
            ...
            for (s = 0; s < RD_SEGS; s = s + 1) begin : g_read_slice
                ...
                (* DONT_TOUCH = "TRUE" *) reg [PTR_W-1:0] wr_ptr_local;
                always @(posedge clk) begin
                    if (push)
                        mem_slice[wr_ptr_local] <= in_data[s*RD_SEG_W +: SLICE_W];
                    if (rst) begin
                        rd_ptr_local <= 0;
                        wr_ptr_local <= 0;
                    end else begin
                        ...
                        if (push)
                            wr_ptr_local <= (wr_ptr_local == DEPTH-1) ? 0 : wr_ptr_local + 1'b1;
                    end
                end
