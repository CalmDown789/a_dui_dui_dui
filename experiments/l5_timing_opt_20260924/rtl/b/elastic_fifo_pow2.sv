...
module elastic_fifo #(
    ...
    localparam integer DEPTH_IS_POW2 = (DEPTH > 1) && ((DEPTH & (DEPTH - 1)) == 0);
            ...
            for (s = 0; s < RD_SEGS; s = s + 1) begin : g_read_slice
                ...
                always @(posedge clk) begin
                    ...
                    if (rst)
                        ...
                        rd_ptr_local <= (DEPTH == 1) ? 0 :
                            (DEPTH_IS_POW2 ? (rd_ptr_local + 1'b1) :
                            ((rd_ptr_local == DEPTH-1) ? 0 : rd_ptr_local + 1'b1));
                end
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (push)
                wr_ptr <= (DEPTH == 1) ? 0 :
                    (DEPTH_IS_POW2 ? (wr_ptr + 1'b1) :
                    ((wr_ptr == DEPTH-1) ? 0 : wr_ptr + 1'b1));
            case ({push,pop})
