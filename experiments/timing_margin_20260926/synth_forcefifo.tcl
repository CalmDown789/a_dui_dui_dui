# Member B 2026-09-26: baseline RTL + targeted post-place L5 FIFO replication.
...
set do_impl 0
set variant "l5splitmem_realrom_150_member_b_forcefifo0926"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_150_member_b_forcefifo0926" }
...
if {$do_impl} {
    ...
    if {$ok} {
        write_checkpoint -force "$stage/base_postphys.dcp"
        report_timing -max_paths 40 -nworst 1 -file "$out_dir/timing_before_forced_replication.rpt"
        set regs [get_cells -quiet -hier -filter {NAME =~ *l5/gk.frontend/fifo/wr_ptr_reg* && REF_NAME =~ FD*}]
        ...
        foreach n $ns {
            ...
            if {[llength $loads] > 128} { lappend force_nets $n }
        }
        close $af
        if {[llength $force_nets] == 0 || [llength $force_nets] > 64} { error "Unexpected force-replication target collection" }
        puts "FORCE_FIFO_NETS: $force_nets"
        phys_opt_design -force_replication_on_nets $force_nets
        report_phys_opt -file "$out_dir/phys_opt_forced.rpt"
        write_checkpoint -force "$stage/forced_postphys.dcp"
    }
