# Member B 2026-09-26: +0.300 ns setup uncertainty during placement only.
...
set do_impl 0
set variant "l5splitmem_realrom_150_member_b_setup0300926"
if {[info exists argv] && [lsearch -exact $argv "acc36"] >= 0} { set variant "acc36_realrom_150_member_b_setup0300926" }
...
if {$do_impl} {
    ...
    if {abs($period - 6.666667) > 0.001} {error "Not the 150 MHz clock"}
    ...
    if {$ok} {
        ...
        # UG949 recommends restoring the original setup uncertainty before route.
        # Original XDC has no user uncertainty. Device jitter remains calculated.
        set_clock_uncertainty -setup -from $cc -to $cc 0.000
        update_timing
        report_clocks -file "$out_dir/clocks_restored_preroute.rpt"
    ...
    if {$ok} { write_checkpoint -force "$stage/postroute.dcp"; set impl_status "COMPLETED" } else { set impl_status "FAILED" }
