# 成员B工作 / Team member B: five independent 6x5 real-A raw layer streams.
param(
...
$xsimWork=Join-Path $env:TEMP "acx750_member_b_five_raw_$stamp"
...
$sources=@(
    ...
    (Join-Path $taskRoot 'tb\five_layer_raw_stream_tb.sv')
)
foreach($source in $sources){Copy-Item -LiteralPath $source -Destination $xsimWork}
Push-Location $xsimWork
try {
    & (Join-Path $VivadoRoot 'bin\xvlog.bat') '-sv' 'same_pad_raster.sv' 'elastic_fifo.sv' 'window_kminus1_bram.sv' 'window_stream_frontend.sv' 'eight_phase_issue.sv' 'phase_mac_array.sv' 'phase_mac_pipeline.sv' 'phase_accumulator.sv' 'mac_issue_stage.sv' 'five_layer_raw_stream_tb.sv'
    if($LASTEXITCODE-ne 0){throw 'xvlog failed'}
    & (Join-Path $VivadoRoot 'bin\xelab.bat') 'five_layer_raw_stream_tb' '-s' 'five_layer_raw_stream_tb_sim'
    if($LASTEXITCODE-ne 0){throw 'xelab failed'}
    & (Join-Path $VivadoRoot 'bin\xsim.bat') 'five_layer_raw_stream_tb_sim' '-runall'
