# 成员B工作 / Team member B: parameter-initialized C-B top RTL regression.
param(
...
$xsimWork=Join-Path $env:TEMP "acx750_member_b_rom_top_$stamp"
...
if($ParameterRomDir){
    ...
    foreach($rom in $romFiles){
        $generated=@(Get-Content -LiteralPath (Join-Path $xsimWork $rom.Name) | Where-Object {$_ -notmatch '^\s*//' -and $_.Trim()})[-1]
        ...
        Copy-Item -LiteralPath $rom.FullName -Destination $xsimWork -Force
    }
}
...
Copy-Item -LiteralPath (Join-Path $taskRoot 'tb\fsrcnn_network_mem_top_tb.sv') -Destination $xsimWork
$tbPath=Join-Path $xsimWork 'fsrcnn_network_mem_top_tb.sv'
...
try {
    $defines=@()
    if($AlwaysReady){$defines=@('-d','TB_ALWAYS_READY')}
    & (Join-Path $VivadoRoot 'bin\xvlog.bat') '-sv' @defines @names 'prelu_requantize.sv' 'fsrcnn_network_mem_top_tb.sv'
